...
当对象的某个属性需要频繁修改时，不适合用string+json，因为它**<u>不够灵活</u>**，每次修改都需要重新将整个对象序列化并赋值；如果**<u>使用hash类型，则可以针对某个属性单独修改</u>**，没有序列化，也不需要修改整个对象。比如，商品的价格、销量、关注数、评价数等可能经常发生变化的属性，就适合存储在hash类型里。
...
![image-20250715175833496](../../../../Library/Application Support/typora-user-images/image-20250715175833496.png)

但是，并不是所有的排行榜都能用list类型实现，只有**<u>定时计算的排行榜才适合使用list类型存储</u>**，与定时计算的排行榜相对应的是实时计算的排行榜，list类型不能支持实时计算的排行榜，下面介绍有序集合sorted set的应用场景时会详细介绍实时计算的排行榜的实现。
...
例如QQ音乐中如果你喜欢一首歌，点个『喜欢』就会将歌曲放到个人收藏夹中，每一个用户做一个收藏的集合，每个收藏的集合存放用户收藏过的歌曲id。（主要原因是set的不重复性，可以自动去重；高效查找，插入、删除、查找的时间复杂度为O(1)，适合快速检查用户是否已经收藏某内容；灵活的集合操作，交集：可实现“共同收藏”功能，并集/差集：用于推荐“你可能喜欢的内容，基于其他用户的收藏”）
...
QQ音乐中有多种实时榜单，比如飙升榜、热歌榜、新歌榜，可以用redis key存储榜单类型，score为点击量，value为歌曲id，用户每点击一首歌曲会更新redis数据，sorted set会依据score即点击量将歌曲id排序。（sorted set类型：key(String), Value(Sorted Set(Score, Value))），实时更新Score，来实现自动排序。
...
![image-20250715183642319](../../../../Library/Application Support/typora-user-images/image-20250715183642319.png)
...
* RDB（Redis Database）持久化：能够在**<u>指定的时间间隔</u>**能对你的数据进行快照存储。
* AOF（Append Only File）持久化：记录每次对服务器写的操作，当服务器重启的时候会重新执行这些命令来恢复原始的数据，AOF命令以redis协议追加保存每次写的操作到文件末尾。Redis还能对AOF文件进行后台重写，使得AOF文件的体积不至于过大。
...
手动触发对应 `save` 命令，会阻塞当前Redis服务器，直到RDB过程完成为止，对于内存比较大的实例会造成长时间阻塞，线上环境不建议使用。
...
（2）父进程执行fork操作创建子进程，fork操作过程中父进程会阻塞，通过`info stats`命令查看latest_fork_usec选项，可以获取最近一个fork操作的耗时，单位为微秒
...
AOF（append only file）持久化：以独立日志的方式**<u>记录每次写命令</u>**， **<u>重启时再重新执行AOF文件中的命令达到恢复数据的目的</u>**。AOF的主要作用是解决了数据持久化的实时性，目前已经是Redis持久化的主流方式。
...
| 维度           | AOF                              | RDB                                                          |
| -------------- | -------------------------------- | ------------------------------------------------------------ |
| **持久化方式** | 记录写命令（增量）               | 定时生成快照（全量）                                         |
| **数据安全性** | 高（`everysec` 最多丢 1 秒数据） | 低（取决于 `save` 配置（`save`过程中会阻塞所有客户端请求），可能丢几分钟数据）；`bgsave`是 fork 一个子进程来执行 RDB 操作，期间主进程还可以照常处理客户端请求，不阻塞服务。生成的 RDB 文件是**fork 子进程瞬间的内存快照** |
| **文件体积**   | 大（包含历史命令）               | 小（二进制压缩）                                             |
| **恢复速度**   | 慢（需重放所有命令）             | 快（直接加载快照）                                           |
| **性能影响**   | 高（频繁磁盘 I/O）               | 低（仅定期 fork 子进程）                                     |
| **适用场景**   | 对数据安全性要求高（如支付系统） | 大规模数据恢复、对性能敏感场景                               |
...
redis 内部使用文件事件处理器 `file event handler`，这个文件事件处理器是`单线程`的，所以 redis 才叫做单线程的模型。它采用 **<u>IO 多路复用机制</u>**同时监听多个 socket，根据 socket 上的事件来选择对应的事件处理器进行处理。
...
* 提供了多种高效的数据结构，这些结构经过优化，能够快速完成各种操作
...
![image-20250720010914729](https://typora-yuanbaoer2.oss-cn-shanghai.aliyuncs.com/img-typora/image-20250720010914729.png)
...
![image-20250720013413374](https://typora-yuanbaoer2.oss-cn-shanghai.aliyuncs.com/img-typora/image-20250720013413374.png)
...
![image-20250720013422704](https://typora-yuanbaoer2.oss-cn-shanghai.aliyuncs.com/img-typora/image-20250720013422704.png)
...
Redis最大占用内存用完之后，如果继续添加数据，如何处理这种情况呢？实际上Redis官方已经定义了**八种策略**来处理这种情况：
...
**数据同步阶段是主从复制最核心的阶段**，根据主从节点当前状态的不同，可以分为**全量复制和增量复制**，后面再讲解这两种复制方式以及psync命令的执行过程，这里不再详述。
...
Redis 的主从复制模式下，一旦主节点由于故障不能提供服务，需要**手动**将从节点晋升为主节点，同时还要通知客户端更新主节点地址，这种故障处理方式从一定程度上是无法接受的。

Redis 2.8 以后提供了 Redis Sentinel 哨兵机制（自动）来解决这个问题。
...
哨兵模式的主要作用在于它能够**自动完成故障发现和故障转移**，并通知客户端，从而实现高可用。哨兵模式通常由一组 Sentinel 节点和一组（或多组）主从复制节点组成。
